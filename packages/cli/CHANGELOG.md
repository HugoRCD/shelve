...
- [#766](https://github.com/HugoRCD/shelve/pull/766) [`ff8d85f`](https://github.com/HugoRCD/shelve/commit/ff8d85f5c26986c89f218d12e596724c914f3069) Thanks [@voidhrithik](https://github.com/voidhrithik)! - From a monorepo root, `push`, `pull`, `diff` and `sync` now run once per package that has its own `shelve.json`, and `--path <dir>` targets a single one. Root `shelve.json` settings reach sub-packages instead of being dropped, though `project` stays per-package.
